...
class ShortenedUrlsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create shortened_url' do
    assert_difference('ShortenedUrl.count') do
      post shortened_urls_url,
           params: { shortened_url: { original_url: @shortened_url.original_url, redirects: @shortened_url.redirects,
                                      short_url: @shortened_url.short_url, title: @shortened_url.title } }
    end
